		...
		<div class="item <?=($bImage ? '' : 'wti')?><?=($bLogo ? ' wlogo' : '')?>">
					...
					<div class="top-info">
						<div class="wrap muted">
							<?if(isset($arResult['DISPLAY_PROPERTIES']['POST']) && strlen($arResult['DISPLAY_PROPERTIES']['POST']['VALUE'])):?>
								<span class="font_upper1"><?=$arResult['DISPLAY_PROPERTIES']['POST']['VALUE']?></span>
							<?endif?>
							...
							<?if(isset($arResult['DISPLAY_ACTIVE_FROM']) && $arResult['DISPLAY_ACTIVE_FROM']):?>
								<span class="date font_upper1"><?=$arResult['DISPLAY_ACTIVE_FROM']?></span>
							<?endif;?>
						</div>
						<div class="title font_lg1"><?=$arResult['NAME'];?></div>
					</div>
				</div>
			</div>
			<div class="bottom-block">
				...
				<div class="close-block">
						...
						<?if($GLOBALS['bMobileForm']):?>
							<a href="<?=$GLOBALS['backUrl']?>" title="Вернуться назад" class="btn btn-default btn-lg wides jqmClose"><?=Loc::getMessage('CLOSE_POPUP');?></a>
						<?else:?>
							<span class="btn btn-default btn-lg wides jqmClose"><?=Loc::getMessage('CLOSE_POPUP');?></span>
						<?endif;?>
				</div>
			</div>
		</div>
