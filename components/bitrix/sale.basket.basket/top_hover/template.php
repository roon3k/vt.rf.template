...
$arTopBasketInfo = array_merge($arCounters["READY"], ["EMPTY_BASKET" => GetMessage("EMPTY_BASKET"), "ALL_SUM" => $arResult['allSum_FORMATED'] ]);
?>
...
<div class="wrap_cont">
	...
	<?if($_POST['firstTime']):?>
		<script src="<?=(((COption::GetOptionString('main', 'use_minified_assets', 'N', $siteID) === 'Y') && file_exists($_SERVER['DOCUMENT_ROOT'].$templateFolder.'/script.min.js')) ? $templateFolder.'/script.min.js?'.filemtime($_SERVER['DOCUMENT_ROOT'].'/'.$templateFolder.'/script.min.js') : $templateFolder.'/script.js?'.filemtime($_SERVER['DOCUMENT_ROOT'].'/'.$templateFolder.'/script.js') )?>" type="text/javascript"></script>
	<?endif;?>
	<?
	...
	?>
		...
		<?$arError = CNext::checkAllowDelivery($arResult["allSum"],CSaleLang::GetLangCurrency(SITE_ID));?>
		...
		<script>	
			...
			if (typeof updateTopBasket === 'function') {
				updateTopBasket(<?=CUtil::PhpToJSObject($arTopBasketInfo)?>);
			}
