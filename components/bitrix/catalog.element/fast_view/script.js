      ...
      if (!!this.params.OFFER_SELECTED) {
        ...
        if ("offers" in this) {
          if (this.offers.length) {
            var sku_id = 0;
            if (window["FAST_VIEW_OID"]) sku_id = window["FAST_VIEW_OID"];
            if (sku_id) {
  ...
  window.JCCatalogElementFast.prototype.ChangeInfo = function () {
    ...
    if (-1 < index) {
      ...
      if (arNextOptions["THEME"]["CHANGE_TITLE_ITEM_DETAIL"] == "Y") {
        ...
        if ($(".fast_view_frame .form_head h2 a").length) {
          $(".fast_view_frame .form_head h2 a").text(this.offers[index].NAME);
        } else {
          $(".fast_view_frame .form_head h2").html($("#fast_view_item .title.hidden").html());
        }
      }
  ...
  window.JCCatalogElementFast.prototype.setBuyBlock = function (th, obj) {
    ...
    if (obj.CONFIG.ACTION !== "NOTHING") {
      if (obj.CONFIG.ACTION == "ADD" && obj.CAN_BUY && obj.SHOW_ONE_CLICK_BUY != "N") {
        var ocb =
          '<span class="btn btn-default white one_click transition_bg" data-offers="Y" data-item="' +
          obj.ID +
          '" data-iblockID="' +
          obj.IBLOCK_ID +
          '" data-quantity="' +
...
$(document).on("click", ".navigation-wrapper-fast-view .fast-view-nav", function () {
  ...
  var container = _this.closest("#popup_iframe_wrapper").find("#fast_view_item");

  container.addClass("sending");

  $.ajax({
    url: arNextOptions["SITE_DIR"] + "ajax/fastViewNav.php",
    type: "POST",
    data: data,
