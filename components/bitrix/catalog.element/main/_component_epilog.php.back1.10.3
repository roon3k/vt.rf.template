...
<?if($arResult["ID"]):?>
									...
									<?if($bShowStoresTab):?>
										<li class="stores_tab<?=(!($iTab++) ? ' active' : '')?>">
											<a href="#stores" data-toggle="tab">
												<span><?=($arParams["TAB_STOCK_NAME"] ? $arParams["TAB_STOCK_NAME"] : GetMessage("STORES_TAB"));?></span>
											</a>
										</li>
									<?endif;?>
								<?endif;?>
	...
	<?elseif($code == 'recomend_goods' && $bViewBlock):?>
		<?if($bBigData):?>
			<div class="wraps hidden_print addon_type drag_block_detail separate_block">
				<hr>
				<h4><?=($arParams['TITLE_SLIDER'] ? $arParams['TITLE_SLIDER'] : GetMessage('RECOMENDATION_TITLE'))?></h4>
				<div class="<?=($blockViewType ? '' : 'bottom_slider');?> specials tab_slider_wrapp custom_type">
					<?if ($blockViewType):?>
						<div class="wraps goods-block with-padding block ajax_load catalog">
					<?else:?>
