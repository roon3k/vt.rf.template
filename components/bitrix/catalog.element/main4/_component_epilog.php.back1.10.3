...
$defaultBlockOrder = 'tizers,complect,nabor,tabs,stores,char,galery,exp_goods,services,gifts,goods,podborki,blog,recomend_goods,assoc_goods';
$defaultTabOrder = 'offers,desc,char,buy,payment,delivery,video,reviews,ask,stores,custom_tab';

$strBlockOrder = isset($arParams["DETAIL_BLOCKS_ORDER"]) ? $arParams["DETAIL_BLOCKS_ORDER"] : $defaultBlockOrder;
$strTabOrder = isset($arParams["DETAIL_BLOCKS_TAB_ORDER"]) ? $arParams["DETAIL_BLOCKS_TAB_ORDER"] : $defaultTabOrder;

$arBlockOrder = explode(",", $strBlockOrder);
$arTabOrder = explode(",", $strTabOrder);
...
<div class="tabs_section type_more">
	...
	$bShowDetailTextTab = ($bShowDetailText  || $bShowDocs ? ++$i : '');
	...
	$bShowStores = ($templateData["STORES"]['USE_STORES'] && $templateData["STORES"]["STORES"] ? ++$i : '');
	...
	$bShowAskTab = ($arParams["SHOW_ASK_BLOCK"] === "Y" ? ++$i : '');
	...
	<?if($code == 'tabs'):?>
		...
		<?if($bShowDetailTextTab || $templateData["OFFERS_INFO"]["OFFERS_MORE"] || $bShowVideoTab || $bShowHowBuyTab || $bShowPaymentTab || $bShowDeliveryTab || $bShowCustomTab || $bShowReviewsTab || $bShowAskTab):?>
					...
					<div class="tabs">
						<ul class="nav nav-tabs">
							...
							<?foreach($arTabOrder as $value):?>
								...
								<?//offers?>
								<?if($value == "offers" && $templateData["OFFERS_INFO"]["OFFERS_MORE"]):?>
								...
								<?if($value == "reviews"):?>
									<?if($bShowReviewsTab):?>
										<li class="product_reviews_tab<?=(!($iTab++) ? ' active' : '')?>">
											<a href="#review" data-toggle="tab"><span><?=($arParams["TAB_REVIEW_NAME"] ? $arParams["TAB_REVIEW_NAME"] : GetMessage("REVIEW_TAB"))?></span> <?$APPLICATION->ShowViewContent('PRODUCT_REVIEWS_COUNT_INFO')?></a>
										</li>
									<?endif;?>
								<?endif;?>
								...
								<?//show ask block?>
								<?if($value == "ask"):?>
									<?if($bShowAskTab && (intVal($arParams["ASK_FORM_ID"]))):?>
					...
					<?foreach($arTabOrder as $value):?>
								...
								<div class="tab-pane <?=(!($iTab++) ? ' active' : '')?>" id="descr">
									...
									<div class="descr-outer-wrapper">
										...
										<?if($arParams["PROPERTIES_DISPLAY_LOCATION"] !== "TAB" && $bShowPropsTab):?>
											<div class="wraps">
												<hr>
												<h4><?= $arParams["TAB_CHAR_NAME"] ?: GetMessage("PROPERTIES_TAB"); ?></h4>
						...
						<?if($value == "char"):?>
							<?if($bShowPropsTab && $arParams["PROPERTIES_DISPLAY_LOCATION"] === "TAB"):?>
								<div class="tab-pane <?=(!($iTab++) ? ' active' : '')?>" id="props">
									<div class="title-tab-heading visible-xs">
										<?=($arParams["TAB_CHAR_NAME"] ? $arParams["TAB_CHAR_NAME"] : GetMessage("PROPERTIES_TAB"));?>
									</div>
									<div>
										<?$APPLICATION->ShowViewContent('PRODUCT_PROPS_INFO')?>
									</div>
						...
						<?if($value == 'offers' && $templateData["OFFERS_INFO"]["OFFERS_MORE"]):?>
							<div class="tab-pane prices_tab<?=(!($iTab++) ? ' active' : '')?>" id="prices_offer">
								<div class="title-tab-heading visible-xs">
									<?=($arParams["TAB_OFFERS_NAME"] ? $arParams["TAB_OFFERS_NAME"] : GetMessage("OFFER_PRICES"));?>
								</div>
						...
						<?if($value == "reviews"):?>
							<?if($bShowReviewsTab):?>
								<div class="tab-pane  <?=$arParams['REVIEWS_VIEW']?> <?=(!($iTab++) ? 'active' : '')?> reviews" id="review">
									<div class="title-tab-heading visible-xs">
						...
						<?if($value == "ask"):?>
							<?if($bShowAskTab && (intVal($arParams["ASK_FORM_ID"]))):?>
								<div class="tab-pane<?=(!($iTab++) ? ' active' : '')?>" id="ask">
									<div class="title-tab-heading visible-xs">
										<?=($arParams["TAB_FAQ_NAME"] ? $arParams["TAB_FAQ_NAME"] : GetMessage('ASK_TAB'))?>
									</div>
									<div class="row">
	...
	<?elseif($code == 'goods' && !$bViewBlock):?>
			...
			<?if(
				...
				<div class="<?=($blockViewType ? '' : 'bottom_slider');?> specials tab_slider_wrapp drag_block_detail">
