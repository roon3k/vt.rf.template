...
<?
$sMapType = CNext::GetFrontParametrValue('CONTACTS_TYPE_MAP');
...
<?if(count($arResult["STORES"]) > 0):?>
	...
	<div class="stores_block_wrap <?if($bCombineMode):?>combine<?endif;?>">
		...
		<?if($bCombineMode):?>
			<div class="stores-title flexbox flexbox--row">
				<div class="stores-title__list stores-title--list stores-title--active">
					<?=\CNext::showIconSvg("stores ncolor normal", SITE_TEMPLATE_PATH."/images/svg/catalog/presence_list.svg");?>
					<?=GetMessage("STORES_LIST");?>
				</div>
				<div class="stores-title__list stores-title--map">
					<?=\CNext::showIconSvg("stores ncolor normal", SITE_TEMPLATE_PATH."/images/svg/catalog/presence_map.svg");?>
		...
		$sStoreType = \Bitrix\Main\Config\Option::get('aspro.next', 'STORES_SOURCE', 'IBLOCK');
		?>

		<?foreach($arResult["STORES"] as $pid => $arProperty):
			$amount = (isset($arProperty['REAL_AMOUNT']) ? $arProperty['REAL_AMOUNT'] : $arProperty['AMOUNT']);
			$bUseLink = (($arShops[$arProperty['ID']]['DETAIL_PAGE_URL'] && $sStoreType == 'IBLOCK') || $sStoreType != 'IBLOCK');
					...
					<div class="stores_text_wrapp <?=(isset($arProperty["IMAGE_ID"]) && !empty($arProperty["IMAGE_ID"]) ? 'image_block' : '')?>">
						...
						<div class="main_info">
							<?if (isset($arProperty["TITLE"])):?>
								<span>
									<?if($bUseLink):?>
										<a class="title_stores" href="<?=$arProperty["URL"]?>" data-storehref="<?=$arProperty["URL"]?>" data-iblockhref="<?=$arShops[$arProperty['ID']]['DETAIL_PAGE_URL']?>">
									<?endif;?>									 
										<?=$storeName?>
									<?if($bUseLink):?>
										</a>
							...
							<?if(isset($arProperty["SCHEDULE"]) && $arProperty["SCHEDULE"]):?><div class="schedule p10"><?=GetMessage('S_SCHEDULE')?>&nbsp;<?=str_replace("&lt;br/&gt;", "<br/>", $arProperty["SCHEDULE"]);?></div><?endif;?>
							<?if(isset($arProperty["EMAIL"]) && $arProperty["EMAIL"]):?><div class="email p10"><?=GetMessage('S_EMAIL')?>&nbsp;<a href="<?='mailto:'.$arProperty["EMAIL"]?>"><?=$arProperty["EMAIL"];?></a></div><?endif;?>
			...
			if($arProperty["GPS_N"] && $arProperty["GPS_N"])
			{
				...
				$arProperty["ADDRESS"] = $storeName;

				if(CNext::GetFrontParametrValue("STORES_SOURCE", $_POST["SITE_ID"]) == 'IBLOCK')
		...
		<?if($bCombineMode || $bMapMode):?>
			...
			<div class="stores-amount-list<?=($bMapMode ? '  stores-amount-list--active' : '');?>">
				<?if($arTmpItems):?>
					...
					<?$arShop=CNext::prepareShopListArray($arTmpItems, $arParams);?>
					<div class="wrapper_block with_title title_left">
						<div class="block_container bordered <?=($nCountItems == 1 ? 'one' : '');?>">
							<div class="items scrollbar" <?=($nCountItems == 1 ? "style='display:none;'" : "")?>>
							...
							<div class="detail_items scrollbar" <?=($nCountItems == 1 ? "style='display:block;'" : "")?>>
								<?foreach($arTmpItems as $arItem):?>
									<?$pos = ($arItem['GPS_N'] && $arItem['GPS_S'] ? $arItem['GPS_N'].','.$arItem['GPS_S'] : '')?>
									<div class="item" <?=($nCountItems == 1 ? "style='display:block;'" : "")?> data-coordinates="<?=$pos;?>" data-id="<?=$arItem['ID']?>">
										<?=CNext::prepareItemMapHtml($arItem, "Y");?>
										<div class="top-close muted svg normal">
					...
					<div class="contacts_map_list">
						...
						<? elseif($sMapType === 'GOOGLE'): ?>
							<?$APPLICATION->IncludeComponent(
								"bitrix:map.google.view",
								"map",
								array(
									"INIT_MAP_TYPE" => "MAP",
									"MAP_DATA" => serialize(array("google_lat" => $arShop["POINTS"]["LAT"], "google_lon" => $arShop["POINTS"]["LON"], "google_scale" => 17, "PLACEMARKS" => $arShop["PLACEMARKS"])),
									"MAP_WIDTH" => "100%",
									"MAP_HEIGHT" => "500",
									"CONTROLS" => array(
										0 => "ZOOM",
