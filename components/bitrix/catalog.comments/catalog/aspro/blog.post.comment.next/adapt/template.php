...
<?if ($arResult['IMAGES'] && !$bAjaxPost):?>
	...
	$arConfig = [
		...
		'spaceBetween' => 10,
		'navigation' => [
			'prevEl' => '.reviews-gallery-block--main .slider-nav--prev',
			'nextEl' => '.reviews-gallery-block--main .slider-nav--next',
		],
		'breakpoints' => [
			'375' => [
				'slidesPerView' => 4,
	...
	?>
	<div class="reviews-gallery-block reviews-gallery-block--main">
		<div class="reviews-gallery-title">
			<span><?=GetMessage('REVIEWS_GALLERY_TITLE');?></span>
		</div>
		<div class="slider-container swiper-nav-offset">
			<div class="swiper slider-solution appear-block" data-plugin-options='<?=\Bitrix\Main\Web\Json::encode($arConfig);?>'>
				<div class="swiper-wrapper">
					<?foreach ($arResult['IMAGES'] as $i => $arPhoto):?>
						<?
						$arImageFileSmallDetail = CFile::ResizeImageGet($arPhoto['FILE_ID'], ["width" => 360, "height" => 360], BX_RESIZE_IMAGE_PROPORTIONAL_ALT)['src'];
						$arImageFileDetail = CFile::GetFileArray($arPhoto['FILE_ID']);
						?>
						<div class="swiper-slide slider-item">
							<a href="<?=$arImageFileDetail['SRC'];?>" class="fancy" rel="reviews-gallery" target="_blank" title="<?=$arImageFileDetail['ORIGINAL_NAME'];?>">
								<img data-id="<?=$arImageFileDetail['ID'];?>" data-src="<?=$arImageFileSmallDetail;?>" src="<?=$arImageFileSmallDetail;?>" class="img-responsive inline swiper-lazy" alt="<?=$arImageFileDetail['ORIGINAL_NAME'];?>" />
							</a>
						</div>
					<?endforeach;?>
				</div>
			...
			<?if (count($arResult['IMAGES']) > 1):?>
				<div class="slider-nav slider-nav--no-auto-hide slider-nav--prev swiper-button-prev">
					<?= CNext::showSpriteIconSvg(SITE_TEMPLATE_PATH . '/images/svg/arrows.svg#left-7-12', '', [
						'WIDTH' => 7, 
						'HEIGHT' => 12
					]); ?>
				</div>
				<div class="slider-nav slider-nav--no-auto-hide slider-nav--next swiper-button-next">
					<?= CNext::showSpriteIconSvg(SITE_TEMPLATE_PATH . '/images/svg/arrows.svg#right-7-12', '', [
						'WIDTH' => 7, 
						'HEIGHT' => 12
					]); ?>
				</div>
							...
							<div class="row form">
								<div class="col-md-6 col-sm-6">
									<div class="form-group animated-labels <?=($_SESSION["blog_user_name"] ? 'input-filed' : '');?>">
										...
										<div class="input">
											<input maxlength="255" size="30" class="form-control" tabindex="3" type="text" name="user_name" id="user_name" value="<?=htmlspecialcharsEx($_SESSION["blog_user_name"]);?>" required>
										</div>
									</div>
								</div>
								<div class="col-md-6 col-sm-6">
						...
						<div class="votes_block nstar big with-text">
							<div class="ratings">
								<div class="inner_rating">
									<?for($i=1;$i<=5;$i++):?>
										<div class="item-rating large" data-message="<?=GetMessage('RATING_MESSAGE_'.$i);?>"></div>
									<?endfor;?>
								</div>
							</div>
				...
				if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
				{
					...
					<div class="blog-comment-info">
						<?if($tabCount > 0 || $comment['PARENT_ID']){
							print_r(CNext::showIconSvg("arrow_answer", SITE_TEMPLATE_PATH."/images/svg/arrow_answer.svg"));
						}?>
						...
						<div class="blog-info__rating">
							<div class="votes_block nstar big with-text">
								<div class="ratings">
									<div class="inner_rating">
										<?for($i=1;$i<=5;$i++):?>
											<div class="item-rating <?=$i<=$comment['UF_ASPRO_COM_RATING'] ? 'filled' : ''?>"></div>
										<?endfor;?>
									</div>
								</div>
							</div>
					...
					<?if (isset($comment['UF_ASPRO_COM_APPROVE']) && $comment['UF_ASPRO_COM_APPROVE']):?>
						<div class="blog-comment-approve alert alert-success alert-success--with-icon font_sxs">
							<?= CNext::showSpriteIconSvg(SITE_TEMPLATE_PATH.'/images/svg/content_icons_sprite.svg#icon-checkmark', 'svg-checkmark', [
								'HEIGHT' => 7, 'WIDTH' => 10
							]);?>
					...
					<div class="blog-comment-content">
						...
						<?if ($comment['IMAGES']):?>
							...
							$arConfig = [
								...
								'spaceBetween' => 10,
								'navigation' => [
									'prevEl' => '#reviews-gallery-block_'.$comment['ID'].' .slider-nav--prev',
									'nextEl' => '#reviews-gallery-block_'.$comment['ID'].' .slider-nav--next',
								],
								'breakpoints' => [
									'375' => [
										'slidesPerView' => 4,
							...
							?>
							<div class="reviews-gallery-block" id="reviews-gallery-block_<?=$comment['ID'];?>">
								<div class="reviews-gallery-title">
									<span><?=GetMessage('REVIEWS_GALLERY_TITLE');?></span>
								</div>
								<div class="slider-container swiper-nav-offset">
									<div class="swiper slider-solution appear-block" data-plugin-options='<?=\Bitrix\Main\Web\Json::encode($arConfig);?>'>
										<div class="swiper-wrapper">
											<?foreach ($comment['IMAGES'] as $i => $arPhoto):?>
												<?
												$arImageFileSmallDetail = CFile::ResizeImageGet($arPhoto['FILE_ID'], ["width" => 360, "height" => 360], BX_RESIZE_IMAGE_PROPORTIONAL_ALT)['src'];
												$arImageFileDetail = CFile::GetFileArray($arPhoto['FILE_ID']);
												?>
												<div class="swiper-slide slider-item">
													<a href="<?=$arImageFileDetail['SRC'];?>" class="fancy" rel="reviews-gallery_<?=$comment['ID'];?>" target="_blank" title="<?=$arImageFileDetail['ORIGINAL_NAME'];?>">
														<img data-id="<?=$arImageFileDetail['ID'];?>" data-src="<?=$arImageFileSmallDetail;?>" src="<?=$arImageFileSmallDetail;?>" class="img-responsive inline swiper-lazy" alt="<?=$arImageFileDetail['ORIGINAL_NAME'];?>" />
													</a>
												</div>
											<?endforeach;?>
