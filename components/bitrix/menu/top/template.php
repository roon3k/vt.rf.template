			...
			<? if ($arParams['HAS_PICTURE'] && $arParams['WIDE_MENU']):
				...
				if (is_array($arImg)): ?>
					<a href="<?= $arParams['ITEM']["LINK"] ?>" title="<?= $arParams['ITEM']["TEXT"] ?>">
						<div class="menu_img"><img src="<?= $arImg["src"] ?>" alt="<?= $arParams['ITEM']["TEXT"] ?>"
								title="<?= $arParams['ITEM']["TEXT"] ?>" /></div>
					</a>
				<? endif; ?>
			<? endif; ?>
			<a href="<?= $arParams['ITEM']["LINK"] ?>" title="<?= $arParams['ITEM']["TEXT"] ?>"><span
					class="name"><?= $arParams['ITEM']["TEXT"] ?></span><?= ($arParams['ITEM']["CHILD"] && $arParams['SHOW_CHILDS'] ? '<span class="arrow"><i></i></span>' : '') ?></a>
			<? if ($arParams['ITEM']["CHILD"] && $arParams['SHOW_CHILDS']): ?>
				...
				<ul class="dropdown-menu toggle_menu">
					<? foreach ($arParams['ITEM']["CHILD"] as $key => $arSubSubItem): ?>
						...
						<li
							class="menu-item <?= (++$key > $iVisibleItemsMenu ? 'collapsed' : ''); ?> <?= ($arSubSubItem["CHILD"] && $bShowChilds ? "dropdown-submenu" : "") ?> <?= ($arSubSubItem["SELECTED"] ? "active" : "") ?>">
							<a href="<?= $arSubSubItem["LINK"] ?>" title="<?= $arSubSubItem["TEXT"] ?>"><span
								...
								<ul class="dropdown-menu">
									<? foreach ($arSubSubItem["CHILD"] as $arSubSubSubItem): ?>
										<li class="menu-item <?= ($arSubSubSubItem["SELECTED"] ? "active" : "") ?>">
											<a href="<?= $arSubSubSubItem["LINK"] ?>" title="<?= $arSubSubSubItem["TEXT"] ?>"><span
					...
					<? if ($iCountChilds > $iVisibleItemsMenu && $arParams['WIDE_MENU']): ?>
						<li><span
								class="colored more_items with_dropdown"><?= \Bitrix\Main\Localization\Loc::getMessage("S_MORE_ITEMS"); ?></span>
						</li>
					<? endif; ?>
				</ul>
			<? endif; ?>
		...
		<table>
			<tr>
				<? foreach ($arResult as $arItem): ?>
					...
					?>
					<td
						class="menu-item unvisible <?= ($arItem["CHILD"] ? "dropdown" : "") ?> <?= (isset($arItem["PARAMS"]["CLASS"]) ? $arItem["PARAMS"]["CLASS"] : ""); ?>  <?= ($arItem["SELECTED"] ? "active" : "") ?>">
						<div class="wrap">
												...
												<div class="customScrollbar scrollbar">
													<div class="menu-navigation__sections">
														<? foreach ($arItem["CHILD"] as $arChild): ?>
															<div
																class="menu-navigation__sections-item<?= ($arChild['SELECTED'] ? " active" : ""); ?>">
																	...
																	<span class="menu-navigation__sections-item-link-inner">
																		<? if ($bShowImg) { ?>
																			<span class="image colored_theme_svg ">
																				...
																				<? else: ?>
																					<img src="<?= $image; ?>" alt="<?= $arChild["NAME"]; ?>"
																						title="<?= $arChild["NAME"] ?>" />
																				<? endif; ?>
																			</span>
																		<? } ?>
											...
											<div class="customScrollbar scrollbar">
												<ul class="menu-wrapper ">
													<? foreach ($arItem["CHILD"] as $arSubItem): ?>
														...
														<li
															class="<?= ($arSubItem["CHILD"] && $bShowChilds ? "dropdown-submenu" : "") ?> <?= ($bShowAsBlock ? "show_as_block" : "") ?> <?= ($arSubItem["SELECTED"] ? "active" : "") ?> <?= ($bHasPicture ? "has_img" : "") ?> parent-items">
															<? if ($arItem['bManyItemsMenu']): ?>
																<div class="subitems-wrapper">
																	<ul class="menu-wrapper">
																		<? foreach ($arSubItem["CHILD"] as $arSubItem2): ?>
																			...
																			?>
																			<li
																				class="<?= ($arSubItem2["CHILD"] && $bShowChilds ? "dropdown-submenu" : "") ?> <?= $bIcon ? 'icon' : '' ?> <?= ($arSubItem2["SELECTED"] ? "active" : "") ?> <?= ($bHasPicture ? "has_img" : "") ?>">
																				<?= showSubItemss([
...
<script>
    ...
    function updateDropdownWidth() {
        $('.menu-item.dropdown').each(function () {
            ...
            // Проверка, что элемент не имеет класс 'catalog'
            if (!$dropdown.hasClass('catalog')) {
                var $toggle = $dropdown.find('.dropdown-toggle');
                var $menu = $dropdown.find('.dropdown-menu');

                if ($toggle.length && $menu.length) {
                    // Установить ширину выпадающего списка равной ширине родительского элемента
    ...
    // Обновить ширину при загрузке страницы
    updateDropdownWidth();

    // Обновить ширину при изменении размера окна
