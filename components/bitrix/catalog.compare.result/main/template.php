	...
	<div class="catalog-compare swipeignore" <?= $bDelCookie ? ' data-delcookie' : '' ?>>
		...
		<div class="catalog-compare__top flexbox flexbox--row justify-content-between align-items-normal">
			...
			<ul class="tabs-head nav nav-tabs hidden">
				<li <?= (!$arResult["DIFFERENT"] ? 'class="active"' : ''); ?>>
					<a rel="nofollow" class="sortbutton<?= (!$arResult["DIFFERENT"] ? ' active' : ''); ?>"
						data-href="?DIFFERENT=N" rel="nofollow"><?= GetMessage("CATALOG_ALL_CHARACTERISTICS"); ?></a>
				</li>
				<li <?= ($arResult["DIFFERENT"] ? 'class="active"' : ''); ?>>
					<a rel="nofollow" class="sortbutton diff <?= ($arResult["DIFFERENT"] ? ' active' : ''); ?>"
						data-href="?DIFFERENT=Y" rel="nofollow"><?= GetMessage("CATALOG_ONLY_DIFFERENT"); ?></a>
				</li>
			</ul>
			...
			?>
			<span class="catalog-compare__clear color-theme-hover muted"
				onclick="CatalogCompareObj.MakeAjaxAction('<?= $GLOBALS['arTheme']['COMPARE_PAGE_URL']['VALUE']; ?>?action=DELETE_FROM_COMPARE_RESULT&ID[]=<?= $arStr ?>', 'Y');">
		...
		<div class="catalog-compare__inner loading">
			...
			<div class="table_compare catalog-block">
				<? foreach ($arResult['SECTIONS'] as $arSection): ?>
					<div class="compare-sections__item <?= $arSection['ID'] === $activeTabId ? 'active' : '' ?>"
									...
									<div class="catalog-block__wrapper height-100">
										<div class="catalog-block__item catalog_item bordered bg-theme-parent-hover border-theme-parent-hover js-notice-block
											...
											<div class="catalog-block__inner flexbox height-100">
												<span
													onclick="CatalogCompareObj.MakeAjaxAction('<?= CUtil::JSEscape($arElement['~DELETE_URL']); ?>', 'Y');"
												...
												<div class="image_wrapper_block js-notice-block__image">
													...
													<? if (!empty($arImage)): ?>
														<a href="<?= $arElement["DETAIL_PAGE_URL"]; ?>">
															<img src="<?= $arImage['SRC']; ?>" alt="<?= $arImage["ALT"]; ?>"
																title="<?= $arImage["TITLE"]; ?>" />
														</a>
													<? endif; ?>
												</div>
												<? $dataItem = CNext::getDataItem($arElement); ?>
												<div class="catalog-block__info flex-1 flexbox justify-content-between"
													data-id="<?= $arElement['ID']; ?>" data-item="<?= $dataItem; ?>">
													<div class="catalog-block__info-top">
														<div class="catalog-block__info-inner">
															<div class="catalog-block__info-inner">
																<?// element title ?>
																<div
																	class="catalog-block__info-title item-title lineclamp-4 height-auto-t600">
																	<a href="<?= $arElement["DETAIL_PAGE_URL"]; ?>"
																		class="dark_link switcher-title js-notice-block__title""><span><?= $name; ?></span></a>
																</div>
															</div>
														</div>
													</div>
													...
													<div class=" catalog-block__info-bottom">
																		<div
																			class="line-block line-block--20 flexbox--wrap justify-content-between">
																			<?// element price ?>
																			<div
																				class="line-block__item catalog-block__info-bottom--margined js-popup-price catalog-block__info-price cost prices">
																				<?
																			...
																			<div
																				class="line-block__item catalog-block__info-bottom--margined">
																				<div class="article_block">
																					<? if (isset($arElement['ARTICLE']) && $arElement['ARTICLE']['VALUE']) { ?>
																						<div> <?= $arElement['ARTICLE']['NAME'] ?> :
																							<?= $arElement['ARTICLE']['VALUE']; ?> </div>
																					<? } ?>
																				</div>
																			</div>
																			<?// element buttons ?>
																			<div
																				class="line-block__item catalog-block__info-bottom--margined catalog-block__info-btn">
																				<div class="counter_wrapp">
																					<? if ($bShowCounterBlock): ?>
																						<div class="counter_block" data-offers="N"
																							data-item="<?= $arElement['ID']; ?>">
																							<span class="minus"
																								id="<?= $arItemIDs['ALL_ITEM_IDS']['QUANTITY_DOWN']; ?>">-</span>
																							<input type="text" class="text"
																								id="<?= $arItemIDs['ALL_ITEM_IDS']['QUANTITY']; ?>"
																								name="<?= $arParams['PRODUCT_QUANTITY_VARIABLE']; ?>"
																								value="<?= $arAddToBasketData['MIN_QUANTITY_BUY']; ?>" />
																							<span class="plus"
																								id="<?= $arItemIDs['ALL_ITEM_IDS']['QUANTITY_UP']; ?>"
																								<?= $arAddToBasketData['MAX_QUANTITY_BUY'] ? ' data-max="' . $arAddToBasketData['MAX_QUANTITY_BUY'] . '"' : ''; ?>>+</span>
																						</div>
																					<? endif; ?>
																					<div class="button_block<?= $bWideButton ? ' wide' : ''; ?>"
													...
													<ul>
														<? if (!empty($arResult["ALL_FIELDS"])) {
															foreach ($arResult["ALL_FIELDS"] as $propCode => $arProp) {
																if (!isset($arResult['FIELDS_REQUIRED'][$propCode])) { ?>
																	<li
																		class="btn btn-transparent <?= ($arProp["IS_DELETED"] != "N" ? 'visible' : ''); ?> btn-xs">
																		<span
																			onclick="CatalogCompareObj.MakeAjaxAction('<?= CUtil::JSEscape($arProp["ACTION_LINK"]); ?>')">+
...
<? endforeach; ?>
													...
													<? if ($arShowOfferFileds): ?>
														<? foreach ($arShowOfferFileds as $code => $arProp): ?>
															<div class="catalog-compare__prop-line font_xs">
																<span
																	class="catalog-compare__prop-name muted"><?= GetMessage("IBLOCK_OFFER_FIELD_" . $code); ?></span>
																<? if ($arResult["ALL_OFFER_FIELDS"][$code]) { ?>
																	<span
																		onclick="CatalogCompareObj.MakeAjaxAction('<?= CUtil::JSEscape($arResult['ALL_OFFER_FIELDS'][$code]['ACTION_LINK']); ?>')"
																		class="remove colored_theme_hover_text stroke-use-grey stroke-theme-use-svg-hover">
																		<i></i>
																	</span>
																<? } ?>
														...
														<? foreach ($arShowProps as $code => $arProp): ?>
															<div class="catalog-compare__prop-line font_xs">
																...
																<? if ($arResult["ALL_PROPERTIES"][$code]): ?>
																	<span
																		onclick="CatalogCompareObj.MakeAjaxAction('<?= CUtil::JSEscape($arResult['ALL_PROPERTIES'][$code]['ACTION_LINK']); ?>')"
																		class="remove colored_theme_hover_text stroke-use-grey stroke-theme-use-svg-hover">
																		<i></i>
																	</span>
																<? endif; ?>
														...
														<? foreach ($arShowOfferProps as $code => $arProp): ?>
															<div class="catalog-compare__prop-line font_xs">
																...
																<? if ($arResult["ALL_OFFER_PROPERTIES"][$code]): ?>
																	<span
																		onclick="CatalogCompareObj.MakeAjaxAction('<?= CUtil::JSEscape($arResult['ALL_OFFER_PROPERTIES'][$code]['ACTION_LINK']); ?>')"
																		class="remove colored_theme_hover_text stroke-use-grey stroke-theme-use-svg-hover">
																		<i></i>
																	</span>
																<? endif; ?>
													...
													<div class="catalog-small__item bordered flexbox flexbox--row height-100">
														<span
															onclick="CatalogCompareObj.MakeAjaxAction('<?= CUtil::JSEscape($arElement['~DELETE_URL']); ?>', 'Y');"
															class="catalog-small__remove remove colored_theme_hover_text stroke-use-grey stroke-theme-use-svg-hover"
															title="<?= GetMessage("CATALOG_REMOVE_PRODUCT"); ?>">
															<i></i>
														</span>
														...
														<div class="catalog-small__img-wrap">
															<a href="<?= $arElement['DETAIL_PAGE_URL']; ?>">
																<img class="img-responsive"
																	src="<?= $arElement['THUMB']['src'] ?? SITE_TEMPLATE_PATH . '/images/no_image_small.png'; ?>"
																	alt="" title="">
															</a>
														</div>
														...
														<div class="catalog-small__name">
															...
															?>
															<a href="<?= $arElement["DETAIL_PAGE_URL"]; ?>"
																class="dark_link switcher-title font_xs"><span
																	class="lineclamp-3"><?= $name; ?></span></a>
														</div>
													</div>
												</div>
											<? }
